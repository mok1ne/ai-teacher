...
## Авторизация и бэкенд (обновлено)

Реальный бэкенд на serverless-функциях (`api/`):
- `POST /api/auth/register` — регистрация (e-mail + пароль, затем имя и возраст). Пароль хранится как scrypt-хэш.
- `POST /api/auth/login` — вход по e-mail и паролю.
- `POST /api/auth/vk` — вход через VK ID (российский сервис авторизации).
- `GET  /api/auth/me` — профиль по токену.
- `POST /api/account/name` | `/password` | `/twofa` — смена имени, пароля, переключение 2FA.

**Хранилище:** Neon (Postgres) через `DATABASE_URL`. Схема — `db/schema.sql`, подключение — `SETUP-NEON.md`. Восстановление пароля: `POST /api/auth/forgot/request` (код на почту) → `POST /api/auth/forgot/verify` (смена + автовход). Прогресс: `GET/POST /api/progress`.

**Соответствие закону об авторизации (РФ, 2026):** вход через иностранные сервисы (Google ID, Apple ID и т.п.) не используется. Доступны: VK ID (российский сервис) и вход по e-mail с паролем (собственная авторизация; сам адрес почты, включая @gmail.com, как логин не запрещён). Для полного соответствия при необходимости добавьте вход по номеру телефона (СМС) или ЕСИA.

**Двухфакторная аутентификация:** флаг включается в настройках; реальная доставка кодов (СМС/TOTP) подключается отдельно.

**Документы** (`/legal/...`): оферта, политика конфиденциальности, согласие на обработку ПДн, правила рекомендательных технологий. Заполните реквизиты `[●]` и согласуйте с юристом.
